/** @type {import('tailwindcss').Config} */
module.exports = {
  content: ["./src/**/*.{js,jsx,ts,tsx}"],
  theme: {
    screens: {
      sm: "576px",
      md: "768px",
      lg: "992px",
      xl: "1200px",
      xxl: "1440px",
      custom: "375px",
    },
    fontFamily: {
      primary: "Montserrat",
      secondary: "Orbitron",
      roboto: "Roboto",
    },
    extend: {
      backgroundImage: {
        OT1: "url('/src/assets/OT1.png')",
        octopath1: "url('/src/assets/octopath1.jpg')",
        octopath2: "url('/src/assets/octopath2.jpg')",
        Header1: "url('/src/assets/Header1.jpg')",
        octopath3: "url('/src/assets/octopath3.jpg')",
        map1: "url('/src/assets/map1.jpg')",
        map11: "url('/src/assets/Map1.1.jpeg')",
        temenos1: "url('/src/assets/TemenosCH.jpg')",
        partitio1: "url('/src/assets/partitioCH.jpg')",
        throne1: "url('/src/assets/Throne.jpg')",
        castti: "url('/src/assets/Castti.jpg')",
        hikari: "url('/src/assets/Hikari.jpg')",
        osvald: "url('/src/assets/Osvald.jpg')",
        Ochette: "url('/src/assets/Ochette.jpg')",
        agnea: "url('/src/assets/Agnea.jpg')",
        hinouema: "url('/src/assets/hinouema.jpg')",
        galaxy: "url('/src/assets/galaxy.jpg')",
        otmobile: "url('/src/assets/Ot.jpg')",
        temenos2: "url('/src/assets/TemenosFull.jpg')",
        ophillia: "url('/src/assets//Ophillia.jpg')",
        alfyn: "url('/src/assets/Alfyn.jpg')",
        cyrus: "url('/src/assets/Cyrus.jpg')",
        haanit: "url('/src/assets/Haanit.jpg')",
        olberic: "url('/src/assets/Olberic.jpg')",
        primrose: "url('/src/assets/Primrose.jpg')",
        therion: "url('/src/assets/Therion.jpg')",
        tressa: "url('/src/assets/Tressa.jpg')",
        dungeon: "url('/src/assets/dungeon.jpg')",
        day: "url('/src/assets/day.png')",
        night: "url('/src/assets/night.png')",
      },
    },
  },
  plugins: [require("@tailwindcss/aspect-ratio")],
};
